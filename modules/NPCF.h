...
#define N6PCF (NBIN*(NBIN-1)*(NBIN-2)*(NBIN-3)*(NBIN-4)/120)
  ...
  for(int x=0;x<N6PCF*nell6;x++) sixpcf[x] += c->sixpcf[x];
    ...
    void save_power(char* out_string, const char* out_folder, Float rmin, Float rmax) {
      ...
      // First create output files
       char out_name[1000];
        snprintf(out_name, sizeof out_name, "%s/%s_3pcf.txt", out_folder, out_string);
...
#ifdef FOURPCF
    {
       ...
       // First create output files
       char out_name2[1000];
        snprintf(out_name2, sizeof out_name2, "%s/%s_4pcf.txt", out_folder, out_string);
       ...
       // First create output files
       char out_name2[1000];
       snprintf(out_name2, sizeof out_name2, "%s/%s_2pcf_mult1.txt", out_folder, out_string);
       FILE * OutFile2 = fopen(out_name2,"w");
      ...
      // First create output files
      char out_name2[1000];
      snprintf(out_name2, sizeof out_name2, "%s/%s_2pcf_mult2.txt", out_folder, out_string);
      FILE * OutFile2 = fopen(out_name2,"w");
       ...
       // First create output files
       char out_name3[1000];
       snprintf(out_name3, sizeof out_name3, "%s/%s_5pcf.txt", out_folder, out_string);
       ...
       // First create output files
       char out_name4[1000];
       snprintf(out_name4, sizeof out_name4, "%s/%s_6pcf.txt", out_folder, out_string);
