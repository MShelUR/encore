...
class Pairs {
  ...
  //   }
    ...
    void save_pairs(const char* out_string, const char* out_dir_name, Float rmin, Float rmax) {
...
void usage() {
    ...
    fprintf(stderr, "   -out <folder>: The output folder.  Default ./output/\n");
...
int main(int argc, char *argv[]) {
  ...
  else if (!strcmp(argv[i],"-out")) ofolder = argv[++i];
...
// Print which N-points are used and check ell-max
  ...
  generate_discon_weights(); // generate the disconnected weights for this specific LMAX, including normalizations. They are stored in weightdiscon1 and weightdiscon2
    ...
    // Save the outputs
    pairs[0].save_pairs(outstr, ofolder, rmin, rmax);
    npcf[0].save_power(outstr, ofolder, rmin, rmax);
